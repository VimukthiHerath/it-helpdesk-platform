...
| Ticket | `PATCH /api/ticket/{id}/status`, `GET /api/ticket/{id}/status-history` | `Agent`, `Administrator` — plus an ownership check for Agents (must be the ticket's own `AssignedTo`, checked in code, not just role). Added in `[[ticket-status-lifecycle]]` (SCRUM-17). |
...
| Assignment | `GET /api/assignments/agents` (view the rotation) | `Agent`, `Administrator` — added Administrator-only in `[[agent-rotation-management]]`, relaxed to include Agent in `[[manual-ticket-reassignment]]` originally so agents could pick a reassignment target. Left as-is (still `Agent`+`Administrator`) even after reassignment itself was narrowed below — viewing the rotation and acting on it are separate questions, and agents still have other reasons to see it. |
| Assignment | `GET /api/assignments` (list all current assignments) | `Agent`, `Administrator` — added in `[[manual-ticket-reassignment]]` (ASSIGN-5 / SCRUM-20). |
| Assignment | `PATCH /api/assignments/{ticketId}/reassign` | `Administrator` — added as `Agent`+`Administrator` in `[[manual-ticket-reassignment]]`, narrowed to Administrator-only shortly after by explicit request: reassigning a ticket away from whoever has it is an admin call, not something one agent should be able to do to another. |
